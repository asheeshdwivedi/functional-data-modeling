...
/** Scala supports tuples, which are a generic way to store two or more pieces of information at the same time. For
  ...
  *
  * Tuples come with lots of free functionality, including constructors, deconstructors, equality and hash code, string
  * representation for debugging, and a copy method.
  *
  * The elements of a tuple are accessed "by index". For example, `("a", 1)._1` retrieves the first element of the
  * tuple, which is the string `"a"`.
  *
  * Tuples are immutable: once constructed, they cannot change. However, there are simple ways to created a new tuple
  * from an existing tuple, in which some element has been "changed".
  *
...
object tuples {
  ...
  /** EXERCISE 3
    *
    * Using both a type alias, and Scala's tuple types, construct a type called `CreditCard` that can hold a credit card
...
object case_class_basics {
  ...
  /** EXERCISE 1
    *
    * Using case classes, construct a type called `Person` that can hold both the name of a `Person` (as a `String`
  ...
  /** EXERCISE 3
    *
    * Using case classes, construct a type called `CreditCard` that can hold a credit card number (as a `String` stored
    * in a field called `number`), a credit card expiration date (as a `java.time.YearMonth` stored in a field called
...
/** Scala's case classes come equipped with useful functionality that all "data classes" should have. In particular,
  * they have equality and hash code built in, and it does exactly what you would expect: operate on the fields of the
  * case class.
  *
  * In addition, case classes have built in copy methods, which can be used to create new values that are modified in
  * some way with respect to an original value.
  */
object case_class_utilities {
  ...
  /** EXERCISE 2
    *
    * Construct and compute the hash codes of two values of type `Person` to see if they are equal to each other. By
  ...
  /** EXERCISE 3
    *
    * Create a copy of the `sherlockHolmes` value, but with the age changed to be 10 years less than it is currently.
    * Create the copy using the `copy` method, which is available on every value of type `Person`. Note that using named
...
/** Both tuples and case classes can be used in pattern matching. Pattern matching can be used to pull out fields from
  * the products and store them in named variables, as well as to selectively match for specific patterns of information
  * stored in a value.
  */
object product_patterns {
  ...
  /** EXERCISE 1
    *
    * In this pattern match on the value `sherlockHolmes`, the name is being extracted and stored into a variable called
    * `name`, while the age is being extracted and stored into a variable called `age`. On the right hand side of the
    * arrow (`=>`), use a Scala `println` statement to print out the name and the age of the specified person.
    */
  def example1 =
  ...
  /** EXERCISE 2
    *
    * Pattern match on this tuple and extract out the name of the product into a variable called `name`, and the price
    * of the product into a variable called `price`. Then use a Scala `println` statement to print out the name and
  ...
  /** EXERCISE 4
    *
    * Pattern matches can contain literal values, in which case those slots in the pattern are matched using the
    * `equals` method of the value. Pattern match on `dilbert` again, but this time, with two cases: the case where the
  ...
  /** EXERCISE 5
    *
    * Every case of a pattern match may contain a conditional expression, in which case the pattern is matched only if
    * both the base pattern matches, and the boolean expression evaluates to true.
    *
  ...
  /** EXERCISE 6
    *
    * Any piece of a pattern match may be captured and placed into a new variable by using the as- pattern syntax `x @
    * ...`, where `x` is any legal variable name. The variable introduced by an as-pattern may be used both in any
    * conditional, or inside the case of the pattern match.
    *
  ...
  /** EXERCISE 7
    *
    * Using the `|` symbol, you can match against two alternatives, providing neither introduces new variables. In the
    * context of case classes, this symbol provides a nice way to look for one among a small number of constant values.
    *
...
/** Scala's case classes can be generic, which means the types defined by case classes may have generic type parameters.
  * This allows building general-purpose and versatile data structures that can have different types "plugged" into them
...
object case_class_generics {
  ...
  /** EXERCISE 3
    *
    * Convert this non-generic class into a generic class, by introducing a new type parameter, called `Body`, which
