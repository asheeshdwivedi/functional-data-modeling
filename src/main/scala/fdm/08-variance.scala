...
/** Scala's type system supports _subtyping_, a feature not common in functional programming languages. Subtyping allows
  * the compiler to see and enforce subset / superset relationships between different types. For example, all dogs are
  * animals, and this fact can be encoded using subtyping in any programming language that supports the feature.
  */
object subtyping {
  trait Animal
  ...
  /** EXERCISE 5
    *
    * In generic data types and methods, the type operators `<:` ("is a subtype of") and `>:` ("is a supertype of") may
    * be used to enforce subtyping / supertyping constraints on type parameters.
    *
...
/** Generic ("parametrically polymorphic") data types with simple, unadorned type parameters are referred to as
  * "invariant". For some invariant generic data type `Box[A]`, there is no relationship between the types `Box[A]` and
  * `Box[B]`, unless `A` and `B` are the same types, in which case, `Box[A]` is the same type as `Box[B]`. If there is a
  * subtyping relationship between `A` and `B`, for example, if `A <: B`, then `Box[A]` is unrelated to (a unique type
...
object invariance {
  ...
  /** EXERCISE 1
    *
    * Assuming you have a `PetHotel` that can book any kind of `Animal`. Use this pet hotel to try to call the
    * `bookRipley` and `bookMidnight` functions, to book these two pets at the hotel.
    *
  ...
  /** EXERCISE 2
    *
    * Assuming you have a `PetDeliveryService` that can deliver `Ripley`, try to use the service to call `acceptRipley`
    * (to accept delivery of `Ripley`), `acceptDog` (to accept delivery of a dog, not necessarily Ripley), and
    * `acceptAnimal` (to accept delivery of an animal, not necessarily a dog).
    *
    * Take note of your findings.
    */
...
/** So-called declaration-site variance is a feature of Scala that allows you to declare, when you define a data type,
  * whether each type parameter should be invariant, covariant, or contravariant. Invariant is the default, and confers
  * no special treatment. Covariance and contravariance, on the other hand, can help improve the usability of generic
  * data types by allowing Scala to safely infer suptyping and supertype relationships between the generic data types
  * when their type parameters have subtyping and supertype relationships.
  *
  * Covariance can be used on any type parameter that appears in "output" position from all methods of a generic data
  * type. The intuition is that covariance on a type parameter means that the data type has a "surplus" (+) of elements
  * of that type "coming out" of it.
  */
object covariance {
  trait Animal
  trait Dog       extends Animal
  ...
  /** EXERCISE 3
    *
    * The rules for covariance imply that in any method that *wants* to take a covariant type parameter as input, it
    * must instead allow any supertype of the type parameter as input.
    *
    * This makes Scala's type system sound. Without it, it would not be safe to allow subtyping on the generic data type
    * based on subtyping of the type parameter.
    *
...
/** Contravariance can be used on any type parameter that appears in "input" position from all methods of a generic data
  * type. The intuition is that contravariance on a type parameter means that the data type has a "deficit" (-) of
  * elements of that type, requiring you feed them in.
  */
object contravariance {
  ...
  /** EXERCISE 3
    *
    * The rules for contravariance imply that in any method that *wants* to take another generic data structure with the
    * same type parameter, it must instead allow that type parameter to be any subtype of the type parameter.
    *
...
/** Type parameters are channels of information: and as channels, they can be used, or ignored. Different types can be
  * used to "ignore" a type parameter depending on whether the parameter is declared as covariant or contravariant (or
  * invariant).
  */
