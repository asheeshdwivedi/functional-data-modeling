...
object sum_modeling {
  ...
  /** EXERCISE 1
    *
    * Using an enum, create a model of a color, which could be `Red`, `Green`, `Blue`, or `Custom`, and if `Custom`,
    * then it should store `red`, `green`, and `blue` components individually, as an integer (`Int`) value.
    */
  sealed trait Color
  object Color {
  ...
  /** EXERCISE 2
    *
    * Using an enum, create a model of an web event, which could be either a page load for a certain URL, a click on a
    * particular button, or a click to a specific URL.
    */
  ...
  /** EXERCISE 3
    *
    * Using an enum, create a model of an age bracket, which could be baby, child, young adult, teenager, adult, mature
  ...
  /** EXERCISE 4
    *
    * Using an enum, create a model of a step in a JSON pipeline, which could be transform, aggregate, or save to file.
    * aggregate.
    */
  type Json
...
/** The following exercises test your ability to model various entities using both case classes and enums.
  */
object mixed_modeling {
  ...
  /** EXERCISE 3
    *
    * Using only case classes and enums, create a model of a page layout for a content-management system, which could
...
object basic_dm_graduation {
  ...
  /** EXERCISE
    *
    * Using case classes and sealed traits (and whatever other data types you like), design a game world that can be
    * used to play a simple text-based role playing game.
    *
